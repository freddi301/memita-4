...
import { AccountId, DeviceKeyPair, generateDeviceKeyPair } from "./cryptography";
...
async function saveDeviceKeyPairs(deviceKeyPairs: Record<string, DeviceKeyPair>): Promise<void> {
  await AsyncStorage.setItem(localStorageKey, JSON.stringify(deviceKeyPairs));
}

async function setDeviceKeyPair(accountId: AccountId, keyPair: DeviceKeyPair): Promise<void> {
...
export async function getDeviceKeyPair(accountId: AccountId): Promise<DeviceKeyPair> {
  const deviceKeyPairs = await loadDeviceKeyPairs();
