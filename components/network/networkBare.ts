...
import { type NetworkFactory, type NetworkInInterface, type NetworkOutInterface } from "../store/store";

export const bareNetworkFactory: NetworkFactory = (networkInImplementation) => {
  const worklet = new Worklet();
  worklet.start("/app.bundle", bundle);

  const [clientReceive, client] = clientFactory<NetworkOutInterface>(async (request) => {
    ...
    for await (const decoded of decodeMultiStream(worklet.IPC)) {
      if ("method" in (decoded as any)) {
        ...
        await serverReceive(
          ...
          decoded as RemoteRequest<NetworkInInterface, keyof NetworkInInterface>,
        );
      } else {
        // TODO validate
        await clientReceive(decoded as RemoteResponse<NetworkOutInterface, keyof NetworkOutInterface>);
      }
    }
  })();
